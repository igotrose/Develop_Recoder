...
static void rk817_bat_rsoc_init(struct rk817_battery_device* battery)
{
    ...
    // 添加：如果电池已经初始化或OCV校准完成，并且RSOC为100%，则直接设置DSOC为100%
    if (battery->is_initialized || battery->is_ocv_calib)
    {
        if (battery->rsoc / 1000 == 100 && battery->dsoc < 100 * 1000)
        {
            battery->dsoc = 100 * 1000;
            BAT_INFO("[will]Battery initialized with full charge, setting DSOC to 100%%\n");
            rk817_bat_save_dsoc(battery, battery->dsoc);
        }
    }
}
...
/* power supply 子系统上报函数 */
static void rk817_bat_power_supply_changed(struct rk817_battery_device* battery)
{
    static int old_soc = -1;
	// 保证显示 SOC 在 0-100 范围内
    if (battery->dsoc > 100 * 1000)
        battery->dsoc = 100 * 1000;
    else if (battery->dsoc < 0)
        battery->dsoc = 0;
	// 如果 SOC 没有变化，并且没有其他需要更新的状态（如充电状态）发生变化，则不触发电源供应状态改变
...
static void rk817_bat_lowpwr_check(struct rk817_battery_device* battery)
{
            ...
            (battery->voltage_avg <= pwr_off_thresd - 50))
        {
            if (base2sec(time) > MINUTE(1))
            {
                ...
                if (battery->voltage_avg <= pwr_off_thresd - 50)
                    battery->dsoc -= 1000;
                printk("[will.huang L2371]low power, soc=%d, current=%d\n",
    ...
    else
    {
        ...
        printk("[will.huang L2378]low power, soc=%d, current=%d\n",
            battery->dsoc, battery->current_avg);
    }

    printk("[will.huang L2380]<%s>. t=%lu, dsoc=%d, current=%d, fake_offline=%d\n",
...
static void rk817_bat_smooth_algorithm(struct rk817_battery_device* battery)
{
    ...
    ydsoc = battery->sm_linek * abs(delta_cap / 10) / DIV(battery->fcc);rk817_bat_smooth_algorithm
...
/* 计算零点模式使用的线性系数，用于把电压变化映射到显示的SOC变化 */
...
static void rk817_bat_finish_algorithm(struct rk817_battery_device* battery)
{
    ...
    if (battery->dsoc < 100 * 1000)
    {
        ...
        // 添加：如果RSOC达到100%，但DSOC未达100%，则强制将DSOC设置为100%
        if ((battery->rsoc / 1000 == 100) && (battery->dsoc < 100 * 1000))
        {
            battery->dsoc = 100 * 1000; // 强制将DSOC设置为100%
            BAT_INFO("[will]Battery fully charged, setting DSOC to 100%%\n");
            rk817_bat_save_dsoc(battery, battery->dsoc); // 保存新的DSOC值
        }
    }
}
/* 充电状态机 */
...
static void rk817_battery_work(struct work_struct* work)
{
	...
	// 将状态推送到 power supply 子系统并打开日志的通知函数
...
{
    // 分配初始化 rk817_battery_device , 分配regmap fields
    ...
    // 初始化延时工作，延时工作的回调是 rk817_battery_work
