...
ADBD_PATH=/usr/sbin/
SERIAL="0123456789"
RNDIS_DEV_MAC="02:12:34:56:78:8a"
...
probe() {
  ...
  if [ "$CLASS" = "ffs.adb" ] ; then
    ...
    mkdir $CVI_GADGET/functions/$CLASS || exit 1
  else
    mkdir $CVI_GADGET/functions/$CLASS.usb$FUNC_NUM || exit 1
  fi
  if [ "$CLASS" = "mass_storage" ] ; then
  ...
  if [ "$CLASS" = "rndis" ] ; then
    ...
    echo $RNDIS_DEV_MAC >$CVI_FUNC/rndis.usb$FUNC_NUM/dev_addr
    echo $RNDIS_HOST_MAC >$CVI_FUNC/rndis.usb$FUNC_NUM/host_addr
    #COMPATIBLE ID
    echo RNDIS >$CVI_FUNC/rndis.usb$FUNC_NUM/os_desc/interface.rndis/compatible_id
    echo 5162001 >$CVI_FUNC/rndis.usb$FUNC_NUM/os_desc/interface.rndis/sub_compatible_id
    #MAKE c.1 THE ONE ASSOCIATED WITH OS DESCRIPTORS
    if [ ! -e $CVI_GADGET/os_desc/c.1 ]; then
      ln -s $CVI_GADGET/configs/c.1 $CVI_GADGET/os_desc/c.1
    fi
    ...
    echo "$RNDIS_LABEL" >$CVI_FUNC/rndis.usb$FUNC_NUM/os_desc/interface.rndis/Label/data
  fi
}
...
start() {
  ...
  if [ -d $CVI_GADGET/functions/ffs.adb ]; then
    ...
    if ! mountpoint -q /dev/usb-ffs/adb; then
      mount -t functionfs adb /dev/usb-ffs/adb || exit 1
    fi

    [ -x $ADBD_PATH/adbd ] || exit 1
...
stop() {
  if [ -d $CVI_GADGET ]; then
    echo "" > $CVI_GADGET/UDC 2>/dev/null || true
  else 
    return 0
  fi
  cleanup_adb_ffs
