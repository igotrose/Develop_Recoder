...
1. 定义变量
    ```bash
    export CVI_DIR=/tmp/usb
    export CVI_GADGET=$CVI_DIR/usb_gadget/cvitek
    export CVI_FUNC=$CVI_GADGET/functions

    export UDC=39010000.usb

    export VID=0x3346
    export PID=0x1009
    export MANUFACTURER="Cvitek"
    export SERIAL="0123456789"
    export PRODUCT="RNDIS"
    ```
2. 清理旧环境
    ```bash
    cd /sys/kernel/config/usb_gadget 2>/dev/null || true
...
5. 创建 gadget 基本描述符
    ...
    mkdir -p $CVI_GADGET/strings/0x409 
    ...
    mkdir -p $CVI_GADGET/configs/c.1/strings/0x409
    echo "config1" > $CVI_GADGET/configs/c.1/strings/0x409/configuration
    echo 120 > $CVI_GADGET/configs/c.1/MaxPower
    ```
6. 创建 RNDIS function
    ```bash
    mkdir -p $CVI_FUNC/rndis.usb0
    ```
7. 固定 MAC 地址
    ```bash
    echo "02:11:22:33:44:55" > "$CVI_FUNC/rndis.usb0/dev_addr"
    echo "02:11:22:33:44:66" > "$CVI_FUNC/rndis.usb0/host_addr"
    ```
8. 写 Windows RNNDIS OS 描述符
    ```bash
    echo 1 > $CVI_GADGET/os_desc/use 
    echo 0xcd > $CVI_GADGET/os_desc/b_vendor_code
    echo MSFT100 > $CVI_GADGET/os_desc/qw_sign

    echo "RNDIS" > $CVI_FUNC/rndis.usb0/os_desc/interface.rndis/compatible_id

    rm -rf $CVI_GADGET/os_desc/c.1
    ln -sf $CVI_AGDGET/configs/c.1 $CVI_GADGET/os_desc
    ```
9. 可选设备显示信息
    ```bash
    mkdir -p $CVI_FUNC/rndis.usb0/os_desc/interface.rndis/Icons
    echo 2 > $CVI_FUNC/rndis.usb0/os_desc/interface.rndis/icons/type
    echo "%SystemRoot%\\system32\\shell32.dll,-233" > "$CVI_FUNC/rndis.usb0/os_desc/interface.rndis/Icons/data"

    mkdir -p "$CVI_FUNC/rndis.usb0/os_desc/interface.rndis/Label"
    echo 1 > "$CVI_FUNC/rndis.usb0/os_desc/interface.rndis/Label/type"
    echo "BM1688 RNDIS" > "$CVI_FUNC/rndis.usb0/os_desc/interface.rndis/Label/data"
    ```
10. link RNDIS 到 config 
    ```bash
    ln -sf $CVI_FUNC/rndis.usb0 $CVI_GADGET/configs/c.1/ 
    ```
11. 绑定到 USB0 UDC 
    ```bash
    echo "" > $CVI_GADGET/UDC 
