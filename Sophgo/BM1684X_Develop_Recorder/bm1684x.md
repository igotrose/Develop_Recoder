...
## 2026-03-27
### OPENSSH 客制化
客户需要将 BM1684X 板卡中的 OpenSSH 升级到较新的 LTS 版本，并把默认 22 端口改为其它端口。该记录后续整理并合并到 2026-08-17 的正式定制笔记中，以 Debian 制包方式作为后续新 SDK 复用的主线。

- 详细记录：[OpenSSH定制笔记.md](20260817/OpenSSH定制笔记.md)
- 集成补丁：[0001-Change-the-SSH-version-to-the-latest-LTS-version.patch](20260817/0001-Change-the-SSH-version-to-the-latest-LTS-version.patch)
- 关键点：
    - OpenSSL / OpenSSH 先生成预编译归档
    - 再制作 `openssh-client`、`openssh-server`、`openssh-sftp-server` 三个 deb 包
    - 最后在 `build_rootp()` 中安装到最终 rootfs
    - `sshd_config` 中配置 `Port 2222`
    - 后续新 SDK 也优先复用 2026-08-17 这套 deb 包流程
## 2026-06-16
### 集成 4/5G 模块
当前两套脚本可以按模块类型才分理解
- 5G 模块走 NCM 拨号流程
