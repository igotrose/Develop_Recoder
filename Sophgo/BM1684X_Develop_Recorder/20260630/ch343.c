...
//if (!(ch343->idProduct == 0x55d2 && ch343->minor == 0)) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
...
// 在这里添加您的serdev注册代码：
#if defined(CONFIG_SERIAL_DEV_BUS) 
	// 如果是CH342设备且是第一个UART接口，则额外注册为serdev设备
	if (ch343->idProduct == 0x55d2 && ch343->iface == 0) {
		struct device *serdev_dev;
		
		serdev_dev = serdev_tty_port_register(&ch343->port,
						      &control_interface->dev,
						      ch343_tty_driver,
						      minor);
		if (!IS_ERR(serdev_dev)) {
			ch343->serdev_dev = serdev_dev;
		//	dev_info(&intf->dev, "Registered as serdev device\n");
		} else {
			dev_warn(&intf->dev, "Failed to register as serdev device: %ld\n",
			PTR_ERR(serdev_dev));
		}
	}
#endif
...
static void ch343_disconnect(struct usb_interface *intf)
{
    ...
    // 如果注册了serdev设备，则注销它
    if (ch343->serdev_dev) {
        serdev_tty_port_unregister(&ch343->port);
        ch343->serdev_dev = NULL;
    }
#endif
